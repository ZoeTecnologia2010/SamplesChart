...
uses Controller.Interfaces, System.Classes, Vcl.Dialogs, System.SysUtils, Vcl.OleCtrls, Vcl.Forms, ActiveX, SHDocVw;
...
type
     ...
     public
          ...
          function AddTitle(aDescription, aValue, aColor: String): iControllerChartDefault;
          function AddValue(aDescription, aValue, aColor: String): iControllerChartDefault;
          procedure ShowChartDonut(aWebBrowser: TWebBrowser);
          procedure ShowChartPie(aWebBrowser: TWebBrowser);
          procedure ShowChartBar(aWebBrowser: TWebBrowser);
     end;
...
procedure TControllerChartDefault.ShowChartBar(aWebBrowser: TWebBrowser);
var
     LocalStream: TStringStream;
begin
     try
          LocalStream := TStringStream.Create(GetChart('barchart_values', aWebBrowser.Width, aWebBrowser.Height));
          //
          LoadStream(aWebBrowser, LocalStream);
     finally
          LocalStream.Free;
...
function TControllerChartDefault.AddTitle(aDescription, aValue, aColor: String): iControllerChartDefault;
begin
     Result := Self;
     //
     FValues.Insert(0, '[' + QuotedStr(aDescription) + ', ' + QuotedStr(aValue) + ', { role: ' + QuotedStr(aColor) + '}]');
end;

function TControllerChartDefault.AddValue(aDescription, aValue, aColor: String): iControllerChartDefault;
begin
     Result := Self;
     //
     FValues.Add('[' + QuotedStr(aDescription) + ', ' + aValue  + ', ' + QuotedStr(aColor) +  ']');
end;
...
begin
     ...
     try
          ...
          aChart.Add('          legend: { position: "none" },');
          aChart.Add('           axes: {x: {0: { side: '+ QuotedStr(FValues.Strings[aLine]) +', label: '+ QuotedStr(FChartTitle) +'} // Top x-axis.}';
          //
          if (aTypeChart = 'donutchart') then aChart.Add('          pieHole: 0.4,');
          //
          aChart.Add('        };');
          //
          if (aTypeChart = 'donutchart') or (aTypeChart = 'piechart') then
               aChart.Add('        var chart = new google.visualization.PieChart(document.getElementById(' + QuotedStr(aTypeChart) + '));')
          else aChart.Add('        var chart = new google.visualization.BarChart(document.getElementById(' + QuotedStr(aTypeChart) + '));');
          //
